...
export default {
    postlist: [ 
        ...
        },
        {
            ...
            "--따라서 서버의 업그레이드 작업을 진행하려면 일일이 충돌을 해결하거나, 새로운 버전에 처음부터 다시 커스텀 작업을 진행하는 방식을 사용해야 하는데, 내가 다니는 회사는 이전 업그레이드 까지는 후자의 방법을 사용했다. 그렇다 나도 안다. \"아니 그러면 업그레이드 할때마다 처음부터 다시 다 작업을 해야돼?\"라는 말이 하고싶을 것이다. 결론부터 말하자면, 실제로 그랬다. 그런데 나를 포함한 몇몇 개발자들은 그게 싫었고 이번 업그레이트 부터는 커스텀 된 버전과 원본 S/W를 메일 rebase해서 충돌되는 부분을 해결하는 자동화 CI를 만들었다. 덕분에 원본에서 바뀐 코드를 바로바로 확인해서 해결할 수 있게 됐다. 이 방식은 업그레이드의 신뢰성과 속도에 충분히 영향을 끼치는데, 커스텀을 한번에 적용하고 제발 문제가 일어나지 말아달라고 빌 필요가 없고 처음부터 다시 커스텀 코드를 적용하는 수고를 할 필요가 없어졌다.  " +
